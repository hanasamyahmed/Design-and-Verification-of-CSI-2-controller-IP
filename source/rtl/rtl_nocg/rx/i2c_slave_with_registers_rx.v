...
//-----------------------------------------------------------------------------
// Module Name   : i2c_slave_with_registers_rx
...
//                 bank of the CSI-2 Receiver. It supports 7-bit slave addressing, 
...
module i2c_slave_with_registers_rx #(
    ...
    always @(posedge i_clk or negedge i_rst_n) begin
        if (!i_rst_n) begin
            ...
            // Handle Transmit Shift Register
            if (tx_load) 
            ...
            // Main datapath and control sequencing based on current state
            case (state)
                ST_ADDR: begin
                ...
                ST_ADDR_ACK: begin
                    if (scl_fall && bit_cnt == 9 && rw_flag) begin 
                        o_reg_rd_en <= 1; tx_load <= 1; r_rd_drive <= 1; 
                    end
                end
                ...
                ST_WDATA: begin
                    if (byte_done) begin 
                        o_reg_wdata <= rx_shift; o_reg_wr_en <= 1; 
                    end
                end
                
                ST_WDATA_ACK: begin
                    if (scl_fall && bit_cnt == 9) o_reg_addr <= o_reg_addr + 1; // Auto-increment address
                end
                
                ST_RDATA: begin
                    if (byte_done) r_rd_drive <= 0; // Release bus for Master to ACK/NACK
                end
                
                ST_RDATA_ACK: begin
                    if (scl_rise && bit_cnt == 8 && sda_d2 == 0) begin // If Master ACKs
                        o_reg_addr <= o_reg_addr + 1; // Auto-increment address
                        o_reg_rd_en <= 1; 
                        tx_load <= 1; 
                        r_rd_drive <= 1; 
                    end
